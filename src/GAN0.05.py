...
X_noise = np.load('../2D_array/X_noise.npy')[:, None]
...
class GAN:
    def __init__(self, t, x, y, p, u, v):
        ...
        self.model_path = '../models/gan/1 0.05 1_/gan{0}.ckpt'
        self.model_load_path = '../models/gan/1 0.05 1_'
        ...
        self.g_loss = 1 * self.l2_Loss + 0.05 * self.pde_residuals + 1 * self.entropy_loss
    ...
    def train(self):
        ...
        log_save_path = '../results/gan/1 0.05 1_/loss.txt'
...
if __name__ == '__main__':
    ...
    X_noise_t = np.tile(X, (1, time_step_train))
    ...
    x_noise = X_noise_t.flatten()[:, None]
    ...
    test_x = X_noise_t[:, random_id].flatten()[:, None]
    test_y = Y_t[:, random_id].flatten()[:, None]
    print(test_t[0])

    g_p, g_u, g_v = gan.predict(test_x, test_y, test_t)
    np.save('../results/gan/1 0.05 1_/pressure{0}.npy'.format(test_t[0]), g_p)
    np.save('../results/gan/1 0.05 1_/velocity_u{0}.npy'.format(test_t[0]), g_u)
    np.save('../results/gan/1 0.05 1_/velocity_v{0}.npy'.format(test_t[0]), g_v)
