...
class GAN:
    def __init__(self, t, x, y, p, u, v):
        ...
        self.g_loss = 1 * self.l2_Loss + 0.1 * self.pde_residuals + 0.5 * self.entropy_loss
...
if __name__ == '__main__':
    ...
    for pre_id in range(400, 500):
        ...
        np.save('../results/pre/gan_1/pressure{0}.npy'.format(test_t[0]), g_p)
        np.save('../results/pre/gan_1/velocity_u{0}.npy'.format(test_t[0]), g_u)
        np.save('../results/pre/gan_1/velocity_v{0}.npy'.format(test_t[0]), g_v)