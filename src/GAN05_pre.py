...
T = np.load('../2D_array/Time.npy')[:, None]
...
class GAN:
    def __init__(self, t, x, y, p, u, v):
        ...
        self.model_path = '../models/gan/{1}/gan{0}.ckpt'
        self.model_load_path = '../models/gan/{0}'.format(path)
        ...
        self.g_loss = 1 * self.l2_Loss + 0.4 * self.pde_residuals + 1 * self.entropy_loss
    ...
    def train(self):
        ...
        log_save_path = '../results/gan/{0}/loss.txt'.format(path)
        ...
        for epoch in range(self.epochs):
            ...
            if (epoch + 1) % 10000 == 0:
                model_path = self.model_path.format(str(epoch + 1), path)
...
if __name__ == '__main__':
    ...
    for pre_id in range(400, 500):
        test_t = T_t[:, pre_id].flatten()[:, None]
        test_x = X_t[:, 0].flatten()[:, None]
        test_y = Y_t[:, 0].flatten()[:, None]
        print(test_t[0])

        g_p, g_u, g_v = gan.predict(test_x, test_y, test_t)
        np.save('../results/pre/gan_0.05/pressure{0}.npy'.format(test_t[0]), g_p)
        np.save('../results/pre/gan_0.05/velocity_u{0}.npy'.format(test_t[0]), g_u)
        np.save('../results/pre/gan_0.05/velocity_v{0}.npy'.format(test_t[0]), g_v)