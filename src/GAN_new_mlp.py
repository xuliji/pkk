...
class GAN:
    def __init__(self, t, x, y, p, u, v):
        ...
        self.model_path = '../models/new_mlp/new_mlp{0}.ckpt'
        self.model_load_path = '../models/new_mlp'
        ...
        self.g_loss = 1 * self.l2_Loss + 0.1 * self.pde_residuals + 1 * self.entropy_loss
    ...
    def train(self):
        ...
        log_save_path = '../results/new_mlp/loss.txt'
...
class Generator:
    def __init__(self, layers):
        ...
        w_u = tf.Variable(xavier_init([self.layers[0], self.layers[1]]), dtype=tf.float32, name='weight')
        b_u = tf.Variable(tf.zeros(shape=[1, self.layers[1]]), dtype=tf.float32, name='bias')
        w_v = tf.Variable(xavier_init([self.layers[0], self.layers[1]]), dtype=tf.float32, name='weight')
        b_v = tf.Variable(tf.zeros(shape=[1, self.layers[1]]), dtype=tf.float32, name='bias')
        ...
        for i in range(self.layers_num - 1):
            ...
            b = tf.Variable(tf.zeros(shape=[1, self.layers[i + 1]]), dtype=tf.float32, name='bias')
    ...
    def nn(self, *inputs):
        ...
        U = tf.sigmoid(tf.matmul(H, self.W_[0]) + self.B_[0])
        V = tf.sigmoid(tf.matmul(H, self.W_[1]) + self.B_[1])
        H = tf.matmul(H, self.W[0]) + self.B[0]

        for i in range(1, self.layers_num - 2):
            H = tf.matmul(H, self.W[i] + self.B[i])
            Z = H*tf.sigmoid(H)
            H = tf.multiply((1 - Z), U) + tf.multiply(Z, V)

        H = tf.matmul(H, self.W[-1]) + self.B[-1]
...
if __name__ == '__main__':
    ...
    # random_id = np.random.choice(time_step_test, 1)
    random_id = 41
    test_t = T_test_t[:, random_id].flatten()[:, None]
    test_x = X_t[:, random_id].flatten()[:, None]
    test_y = Y_t[:, random_id].flatten()[:, None]
    print(test_t[0])

    g_p, g_u, g_v = gan.predict(test_x, test_y, test_t)
    np.save('../results/new_mlp/pressure{0}.npy'.format(test_t[0]), g_p)
    np.save('../results/new_mlp/velocity_u{0}.npy'.format(test_t[0]), g_u)
    np.save('../results/new_mlp/velocity_v{0}.npy'.format(test_t[0]), g_v)
